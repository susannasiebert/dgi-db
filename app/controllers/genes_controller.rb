class GenesController < ApplicationController
  ...
  def show
    gene = Gene.for_show.where('lower(genes.name) = ?', params[:name].downcase)
  ...
  def druggable_gene_category
    sources = if params[:sources]
                  ...
                  Source.source_names_with_category_information
                end
  ...
  def druggable_gene_categories
    @sources = Source.source_names_with_category_information
  ...
  def validate_search_request(params)
    ...
    params[:gene_categories] = GeneClaimCategory.all_category_names unless params[:gene_categories]
    params[:sources] = Source.potentially_druggable_source_names unless params[:sources]
    params[:source_trust_levels] = SourceTrustLevel.all_trust_levels unless params[:source_trust_levels]
